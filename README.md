...
`npm run package` builds the upload zip; `npm run shots` regenerates the four
1280x800 screenshots in `docs/screenshots` from Wikipedia's article on
marginalia. `docs/store-listing.md` holds the listing text, the per-permission
justifications and every privacy answer, `PRIVACY.md` is the policy a reviewer
asks for a URL to, and `docs/publishing.md` tracks what is left — a developer
registration and the upload itself. Icons come from `tools/make-icons.py`,
since no image library is installed here.
